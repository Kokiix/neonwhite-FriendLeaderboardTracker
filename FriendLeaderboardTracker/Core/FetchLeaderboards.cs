...
static class FetchLeaderboards
{
            ...
            foreach (var mission in campaign.missionData)
            {
                ...
                foreach (var level in mission.levels)
                {
                    if (LEVEL_NAMES.ContainsKey(level.levelID))
                    {
                        ...
                        FetchSingleLB(level.levelID);
                        yield return new WaitWhile(() => _fetchInProgress);
    ...
    static bool _fetchInProgress;

    internal static void FetchSingleLB(string levelID)
    {
    ...
    // This is needed because the displayname in LevelData isn't always accurate
